...
def resolve_auth(context: ResolutionContext, auth: BaseModel) -> AirflowAuthBackend:
    if auth.type == "basic_auth":
        resolved_model = resolve_fields(auth, ResolvedAirflowBasicAuthBackend, context)
        return AirflowBasicAuthBackend(
            webserver_url=resolved_model["webserver_url"],
            username=resolved_model["username"],
            password=resolved_model["password"],
        )
    elif auth.type == "mwaa":
        resolved_model = resolve_fields(auth, ResolvedAirflowMwaaAuthBackend, context)
        try:
        ...
        boto_session = boto3.Session(
            region_name=resolved_model.get("region_name"),
            profile_name=resolved_model.get("profile_name"),
            aws_access_key_id=resolved_model.get("aws_access_key_id"),
            aws_secret_access_key=resolved_model.get("aws_secret_access_key"),
            aws_session_token=resolved_model.get("aws_session_token"),
            aws_account_id=resolved_model.get("aws_account_id"),
        )
        return MwaaSessionAuthBackend(
            mwaa_client=boto_session.client("mwaa"),
            env_name=resolved_model["env_name"],
        )
    else:
